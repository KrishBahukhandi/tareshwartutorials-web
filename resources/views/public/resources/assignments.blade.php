...
@section('title', 'Assignments Class ' . $classLevel . ($subject ? ' — ' . $subject : '') . ' | Tareshwar Tutorials')
@section('description', 'Free downloadable practice assignments for Class ' . $classLevel . ($subject ? ' ' . $subject : '') . ', covering chapter-wise NCERT Exemplar problems.')
...
<section class="bg-primary py-xl">
    <div class="max-w-container-max mx-auto px-gutter">
        <p class="text-sm text-blue-200/70 mb-1">Academic Resources</p>
        <h1 class="text-3xl font-extrabold text-white mb-2">
            CBSE Class {{ $classLevel }} Assignments{{ $subject ? ': ' . $subject : '' }}
        </h1>
        <p class="text-blue-100/70 text-sm max-w-3xl">Download authentic chapter-wise assignments to boost your subject understanding. Consistent practice is the most effective way to master complex topics and score high marks.</p>
    </div>
</section>
...
<div class="bg-white border-b border-outline-variant sticky top-[57px] z-40 shadow-sm">
    <div class="max-w-container-max mx-auto px-gutter flex gap-0 overflow-x-auto">
        @foreach(['10' => 'Class 10 Assignments', '12' => 'Class 12 Assignments'] as $cls => $label)
            <a href="{{ route('assignments.index', ['class' => $cls]) }}"
...
<div class="bg-surface-container-low border-b border-outline-variant">
    <div class="max-w-container-max mx-auto px-gutter py-sm flex flex-wrap gap-sm">
        @foreach($subjects as $sub)
            <a href="{{ route('assignments.index', ['class' => $classLevel, 'subject' => $sub]) }}"
    ...
    <div class="max-w-container-max mx-auto px-gutter">
        ...
        @if(! $subject)
            ...
            <div class="max-w-3xl mx-auto mt-lg">
                <h2 class="text-xl font-bold text-primary mb-lg text-center">Choose a subject to view its assignments</h2>
                        ...
                        <a href="{{ route('assignments.index', ['class' => $classLevel, 'subject' => $sub]) }}"
                           class="bg-white rounded-xl border border-outline-variant shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 p-lg flex items-center gap-md group">
                            <span class="w-12 h-12 rounded-xl bg-secondary-fixed flex items-center justify-center shrink-0">
                                <span class="material-symbols-outlined text-[24px] text-secondary">assignment</span>
                            </span>
                            <div class="flex-1">
                                <h3 class="font-bold text-primary group-hover:text-secondary transition-colors">{{ $sub }}</h3>
                                <p class="text-xs text-on-surface-variant mt-0.5">
                                    {{ $count > 0 ? $count . ' ' . Str::plural('chapter', $count) . ' available' : 'Coming soon' }}
                                </p>
                            </div>
        ...
        @elseif($resources->isEmpty())
            {{-- Empty state: subject chosen but no assignments yet --}}
            <div class="text-center py-xxl bg-white rounded-2xl border border-outline-variant shadow-sm max-w-2xl mx-auto mt-lg">
                <div class="w-20 h-20 bg-secondary-fixed rounded-full flex items-center justify-center mx-auto mb-lg">
                    <span class="material-symbols-outlined text-[40px] text-secondary">assignment</span>
                </div>
                <h3 class="text-xl font-bold text-primary mb-sm">{{ $subject }} assignments coming soon!</h3>
                <p class="text-on-surface-variant text-sm max-w-sm mx-auto mb-lg">
                    We're currently digitizing and uploading {{ $subject }} chapter-wise assignments for Class {{ $classLevel }}. Check back soon!
                </p>
                <a href="{{ route('assignments.index', ['class' => $classLevel]) }}" class="inline-flex items-center gap-1 text-sm font-semibold text-primary hover:underline">
        ...
        @else
            ...
            <div class="mt-lg">
                <a href="{{ route('assignments.index', ['class' => $classLevel]) }}" class="inline-flex items-center gap-1 text-sm font-semibold text-primary hover:underline mb-md">
                    ← All subjects
                </a>

                <div class="mb-4">
                    <h2 class="text-2xl font-extrabold text-primary mb-2">
                        Class {{ $classLevel }} {{ $subject }} Assignments
                    </h2>
                    <p class="text-sm text-on-surface-variant">
                        Download free PDF of {{ $subject }} chapter-wise assignments to practice for your board exams.
                    </p>
                </div>
                ...
                <div class="bg-white rounded-xl border border-outline-variant shadow-sm overflow-hidden">
                    <div class="hidden sm:grid grid-cols-12 gap-4 px-6 py-3 bg-gray-50 border-b border-outline-variant text-xs font-bold text-gray-500 uppercase tracking-wider">
                        <div class="col-span-8">Assignment</div>
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <a href="{{ route('notes.show', $resource) }}" class="text-base font-bold text-[#1e3a5f] group-hover:text-blue-700 transition-colors">
                                                    {{ $resource->chapter ?? $resource->title }}
                                                </a>
                ...
                <div class="mt-xl bg-gradient-to-br from-primary to-[#152a45] rounded-2xl p-8 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between gap-6 shadow-lg">
                    <div>
                        ...
                        <p class="text-blue-100 text-sm">Join our specialized board prep batches where our experts break down every assignment step-by-step.</p>
                    </div>
