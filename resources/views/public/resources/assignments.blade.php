...
<section class="bg-primary py-xl">
    <div class="max-w-container-max mx-auto px-gutter">
        ...
        <h1 class="text-3xl font-extrabold text-white mb-2">
            CBSE Class {{ $classLevel }} Assignments
        </h1>
    ...
    <div class="max-w-container-max mx-auto px-gutter">
        ...
        @if($resources->isEmpty())
            ...
            <div class="text-center py-xxl bg-white rounded-2xl border border-outline-variant shadow-sm max-w-2xl mx-auto mt-lg">
                ...
                <h3 class="text-xl font-bold text-primary mb-sm">Assignments coming soon!</h3>
                <p class="text-on-surface-variant text-sm max-w-sm mx-auto">
                    We're currently digitizing and uploading the past 10 years of board papers for Class {{ $classLevel }}. Check back soon!
                </p>
            </div>
        @else
            @php
            ...
            <div class="flex flex-col md:flex-row gap-xl mt-lg">
                
                {{-- Left Sidebar: Sticky Table of Contents --}}
                <aside class="md:w-1/4 shrink-0">
                    <div class="sticky top-[140px] bg-white rounded-xl shadow-sm border border-outline-variant p-5">
                        <h3 class="font-bold text-primary text-lg mb-4 border-b border-outline-variant pb-2">Subjects</h3>
                        <ul class="space-y-1">
                            @foreach($grouped as $subjectName => $items)
                                <li>
                                    <a href="#subject-{{ Str::slug($subjectName) }}" 
                                       class="flex items-center justify-between p-2 rounded-lg text-sm font-medium text-on-surface-variant hover:bg-primary/5 hover:text-primary transition-colors">
                                        <span>{{ $subjectName }}</span>
                                        <span class="bg-gray-100 text-gray-500 text-xs px-2 py-0.5 rounded-full">{{ $items->count() }}</span>
                                    </a>
                                </li>
                            @endforeach
                        </ul>
                ...
                {{-- Right Content: Lists of Papers --}}
                <main class="md:w-3/4">
                    @foreach($grouped as $subjectName => $items)
                        <div id="subject-{{ Str::slug($subjectName) }}" class="mb-xxl scroll-mt-[140px]">
                            
                            {{-- Subject Header --}}
                            <div class="mb-4">
                                <h2 class="text-2xl font-extrabold text-primary mb-2">
                                    Class {{ $classLevel }} {{ $subjectName }} Assignments
                                </h2>
                                <p class="text-sm text-on-surface-variant">
                                    Download free PDF of {{ $subjectName }} chapter-wise assignments with solutions to practice for your board exams.
                                </p>
                            </div>
